...
regstwo reg     d0-d6/a0-a4     the rest
        ...
        GENIF   q68 = 0         ; Q68 has no gap interrupt
...
ss_int2
        ...
        GENIF   q68 = 0        ; no IPC, tx or gap interrupts on Q68
        ...
        GENIF   q68 <> 0        ; this section for Q68 only
        
        moveq   #pc.intrf,d7
        and.b   pc_intr,d7      ; only test frame interrupt bit!
        bne.s   frint           ; it's a frame interrupt

* on Q68, if not frame interrupt, it's an external interrupt so continue
* (don't trust the exint bit, earlier Q68s don't set it!)
...
is_sup
        ...
        GENIF   q68 = 0         ; no ipc or tx interrupts on Q68
