...
* 16-bit (MODE 33) flavours by JB, August 2023

        xdef    cs_fill16,cs_over16,cs_pan16,cs_recol16,cs_scrol16
        xdef    getmask16
...
; Get colour masks for 16-bit colour mode, given the main and contrast colour
; at (a1) and 1(a1) respectively
; This is called twice here for even and odd lines

getmask16
        moveq   #0,d7
        move.b  (a1)+,d7        ; get main colour
        add.w   d7,d7
        move.w  col_q68(pc,d7.w),d7 ; and mask
        swap    d7              ; swap to high word
        move.b  (a1)+,d7        ; contrast colour
        add.w   d7,d7
        move.w  col_q68(pc,d7.w),d7 ; and mask
        rts

; colour masks for Q68 hi-colour modes ('MODE 33')
; the colour bits are coded GGGGGRRRRRBBBBBC per pixel
; (5 bits each for G, R and B plus one common bit C (not used))
...
;        move.w  (a1),d6         put even lines in both halves of d6
        bsr     getmask16       ; get mask in d7
        move.l  d7,d6           ; transfer to d6 for even lines
        bsr     getmask16       ; now get mask for odd lines in d7
set_line
        btst    #0,d1
        bne.s   set_addr
...
set_addr
        ...
        move.w  d2,d3
        subq.w  #1,d3
        asr.w   #1,d3           ; rhs rounded down to long words
        move.w  d0,d4
        lsr.w   #1,d4           ; x origin in longwords
...
;       and.w   #15,d2
        and.w   #1,d2           ; test lsb of position
        beq.s   mask_rts        ; if even, done
        clr.w   d4              ; if odd, clear msw
        swap    d4
...
cs_over16
        movem.l reg_list,-(sp)
        bsr     set_colm        set up colour masks and pointers
...
cs_fill16
        movem.l reg_list,-(sp)
        bsr     set_colm        set up colour masks
fill
