...
;        movem.l (a1),d6-d7      fetch patterns
        jsr     getmask16(pc)
        move.l  d7,d6           ; strip colour mask
        jsr     getmask16(pc)   ; ink colour mask
        btst    #0,d1           odd row?
...
inc_ok
        ...
        ror.l   #8,d2           ; d2 = $80000000 .. $ff0000ff
...
;        and.w   d2,d3           mask character with field
        ror.w   d1,d3           ; get bit from character column
        clr.w   d3
        subx.w  d3,d3           ; set D3 to $0000 or $ffff 
